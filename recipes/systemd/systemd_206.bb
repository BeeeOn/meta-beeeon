# Fast and dirty way of systemd fixex
...
do_install() {
	...
	if ${@base_contains('DISTRO_FEATURES','sysvinit','true','false',d)}; then
        ...
        # Handle our settings
        ln -sf multi-user.target ${D}${systemd_unitdir}/system/default.target
        ln -sf /dev/null ${D}${systemd_unitdir}/system/proc-sys-fs-binfmt_misc.automount
        ln -sf /dev/null ${D}${systemd_unitdir}/system/sys-module-fuse.device
        ln -sf /dev/null ${D}${systemd_unitdir}/system/sys-fs-fuse-connections.mount

        # Various configuration
        sed -i -e 's/^#\?RuntimeWatchdogSec=.*/RuntimeWatchdogSec=20s/' ${D}${sysconfdir}/systemd/system.conf
        sed -i -e 's/^#\?ShutdownWatchdogSec=.*/ShutdownWatchdogSec=23s/' ${D}${sysconfdir}/systemd/system.conf
